...
className={manualEditMode ? `manual-edit-workspace${resizingPanel ? ' is-resizing' : ''}` : 'comment-preview-layer'}
             ref={workspaceRef}
             style={manualEditMode ? { gridTemplateColumns: `${layersWidth}px ${MANUAL_EDIT_HANDLE_WIDTH}px ${editorWidth}px ${MANUAL_EDIT_HANDLE_WIDTH}px minmax(${previewPanelWidth}px, 1fr)` } : undefined}
           >
             {manualEditMode ? (
               <>
                 <ManualEditPanel
